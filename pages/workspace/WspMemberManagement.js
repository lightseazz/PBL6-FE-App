  ...
  async function onPressDelete() {
    try {
      ...
      if (!successStatusCodes.includes(String(response.status))) {
        setSnackBar({ isVisible: true, message: "you are not authorized to delete member", type: "failed" });
        return;
      }

      const allUsersResponse = await getUserByWorkspacdIdApi(workspaceId);
      setSnackBar({ isVisible: true, message: "delete member successfully", type: "success" });
      ...
      setSnackBar({ isVisible: true, message: "failed delete member", type: "failed" });
